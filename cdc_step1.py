...
def start_gatekeeper():
    print("\n" + "="*45)
    print("      CODAC COIN PORTAL: THE GATEKEEPER        ")
    ...
    # 4. SAVE TO DATABASE
    conn = sqlite3.connect('codac_portal/mother_tree.db')
    cursor = conn.cursor()
    ...
    cursor.execute("INSERT INTO members VALUES (?, ?, ?, ?, ?, ?, ?)", 
                   (member_name, selected_ent, f_name, m_name, ben_name, ben_rel, "PENDING 8-CODAC"))
    conn.commit()
    conn.close()
