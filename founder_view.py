...
def show_founder_stats():
    try:
        ...
        print("\n" + "="*50)
        print("🏛️  CDC SOVEREIGN FOUNDER CONTROL PANEL")
        print("="*50)

        # 1. Personal Wallet Check
        cur.execute("SELECT reward_wallet, cdc_coin_wallet FROM portal_citizens WHERE binary_id = 386;")
        wallet = cur.fetchone()
        
        if wallet:
            print(f"💰 PERSONAL REWARDS : {wallet[0]:,} USDT")
            print(f"🪙 CDC COINS       : {wallet[1]:,} CDC")
        else:
