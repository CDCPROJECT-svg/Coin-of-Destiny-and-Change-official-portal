...
const printCrashReport = ({ error, logPath }) => {
	...
	if (logPath) {
		...
		console.error(`${jobPrefix}${ansi_colors.default.cyan("📄 Crash log saved to:")} ${ansi_colors.default.gray(logPath)}`);
	}
};
const shouldReportCrash = async ({ error, isInteractive }) => {
	...
	return new Promise((resolve) => {
		const jobPrefix = ansi_colors.default.gray("[root] ");
		console.log(`${jobPrefix}${ansi_colors.default.yellow("📢 Open a GitHub issue with crash details? (y/N):")}`);
		...
		const ensureCompatibleVersion = (compatibilityVersion) => {
			...
			if (compatibilityVersion === 4 || compatibilityVersion === 3 || compatibilityVersion === "mini") {
				if (!context.package.satisfies(version, ">=3.25.0 <5.0.0")) {
					...
					console.warn(`🔌 ${ansi_colors.default.yellow("Warning:")} Installed ${ansi_colors.default.cyan(packageName)} ${ansi_colors.default.cyan(`v${version.version}`)} does not support compatibility version ${ansi_colors.default.yellow(String(compatibilityVersion))}, using ${ansi_colors.default.yellow(String(compatibleVersion))}.`);
